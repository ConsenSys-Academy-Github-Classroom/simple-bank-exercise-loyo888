...
contract SimpleBank {
    ...
    event LogEnrolled(address accAddress);

    // Add 2 arguments for this event, an accountAddress and an amount
    event LogDepositMade(address accAddress, uint256 amount);
    ...
    function enroll() public returns (bool) {
        ...
        address accAddress = msg.sender;
        enrolled[accAddress] = true;
        emit LogEnrolled(accAddress);
        return enrolled[accAddress];
    }
    ...
    function deposit() public payable returns (uint256) {
        ...
        address accAddress = msg.sender;
        require(enrolled[accAddress] == true, "user is not enrolled");
        // 3. Add the amount to the user's balance. Hint: the amount can be
        //    accessed from of the global variable `msg`
        balances[accAddress] = balances[accAddress] + msg.value;

        // 4. Emit the appropriate event associated with this function
        emit LogDepositMade(accAddress, msg.value);
    ...
    function withdraw(uint256 withdrawAmount) public returns (uint256) {
        ...
        address payable accAddress = msg.sender;
        require(
            balances[accAddress] >= withdrawAmount,
            "insufficient amount to withdraw"
        );

        // 2. Transfer Eth to the sender and decrement the withdrawal amount from
        //    sender's balance
        balances[accAddress] = balances[accAddress] - withdrawAmount;
        accAddress.transfer(withdrawAmount);
        // 3. Emit the appropriate event for this message
        emit LogWithdrawal(accAddress, withdrawAmount, balances[accAddress]);
        return balances[accAddress];
    }
}
